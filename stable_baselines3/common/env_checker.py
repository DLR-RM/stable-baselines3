...
def _check_returned_values(
    ...
    if isinstance(observation_space, spaces.Dict):
        ...
        for key in obs.keys():
            if key not in observation_space.spaces.keys():
                raise AssertionError(
                    f'The key "{key}" in the observation returned by reset() is not present in the observation space'
                )
    ...
    if isinstance(observation_space, spaces.Dict):
        ...
        for key in obs.keys():
            if key not in observation_space.spaces.keys():
                raise AssertionError(
                    f'The key "{key}" in the observation returned by step() is not present in the observation space'
                )
