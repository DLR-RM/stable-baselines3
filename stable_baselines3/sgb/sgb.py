...
class GDB(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        n_steps: int = 2048,
        n_epochs: int = 1, ## not recommended to be > 1
        gamma: float = 0.99,
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                for i in range(rollout_data.actions.shape[0]):
                    state = rollout_data.observations[i]
                    prev_action = rollout_data.actions[i]

                    action = self.policy(state.unsqueeze(0).to(self.device))[0]
