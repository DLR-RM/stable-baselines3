...
- Added `deserialization_mode` parameter to all load methods (`"safe"` or `"legacy"`) to mitigate dserialization of Untrusted Data. `"safe"` mode uses a restricted unpickler that only allows a fixed allowlist of known-safe SB3/gymnasium/numpy types.
