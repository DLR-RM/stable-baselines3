...
from stable_baselines3.common.evaluation import evaluate_policy, evaluate_mo_policy
...
class MoEvalCallback(EvalCallback):

    def __init__(self, eval_env: Union[gym.Env, VecEnv], n_objectives: int, **kwargs):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths = evaluate_mo_policy(
                self.model,
                self.eval_env,
                self.n_objectives,
                n_eval_episodes=self.n_eval_episodes,
            ...
            if self.verbose >= 1:
                print(f"Eval num_timesteps={self.num_timesteps}, " f"episode_reward={mean_reward.round(2)} +/- {std_reward.round(2)}")
                print(f"Episode length: {mean_ep_length:.2f} +/- {std_ep_length:.2f}")
            # Add to current Logger
            for j in range(self.n_objectives):
                self.logger.record(f"eval/mean_reward_{j}", float(mean_reward[j]))
            self.logger.record("eval/mean_ep_length", mean_ep_length)
