...
def test_batch_size_1_not_nan():
    ...
    rollout_buffer = buffers.RolloutBuffer(buffer_size=2, observation_space=obs_space, action_space=action_space)
    for _ in range(2):
        rollout_buffer.add(
            obs=np.zeros(1),
            action=np.zeros(1),
            reward=np.zeros(1),
            episode_start=np.zeros(1),
            value=th.zeros(1),
            log_prob=th.zeros(1),
        )
    ...
    self.policy.evaluate_actions.return_value = (th.zeros(1, requires_grad=True), th.zeros(1, requires_grad=True), th.zeros(1))
