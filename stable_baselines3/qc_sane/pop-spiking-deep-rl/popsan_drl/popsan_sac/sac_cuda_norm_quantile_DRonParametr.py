...
def spike_sac(env_fn, actor_critic=SpikeActorDeepCritic, ac_kwargs=dict(), seed=100,
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            ...
            pi_, logp_pi_ = ac_targ.popsan(o, batch_size)#-->old
            pi, logp_pi = ac.popsan(o, batch_size)#-->new

            rho=torch.exp(logp_pi_)/torch.exp(logp_pi)
        ...
        temp1=torch.sum(torch.stack(qf1_losses,dim=-1),-1)
        loss_q1 = torch.mean(temp1*rho) #sum of 3 loss and mean over samples
        #print(temp1.shape,rho.shape)
        ...
        temp2=torch.sum(torch.stack(qf2_losses,dim=-1),-1)
        loss_q2 = torch.mean(temp2*rho)


        #print("rho",rho,loss_q1)
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            with open(model_dir + '/' + "terminal" + str(model_idx) + '_out.csv', 'w', newline='\n') as csvfile:
                fieldnames = ["t","Mean_rew"]
                writer3 = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer3.writeheader()
                writer3.writerow({'t':t+1, 'Mean_rew':test_mean_reward})
