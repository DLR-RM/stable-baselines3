...
from core_cuda import MLPQFunction_quantile
...
def spike_sac(env_fn, actor_critic=SpikeActorDeepCritic, ac_kwargs=dict(), seed=100,
    ...
    def compute_loss_pi(data):
        ...
        for actor_index in range(ac_kwargs["num_actor"]):
            ...
            #print(batch_size) 
            exec("loss_pi%d,inf%d = %s"%(actor_index+1,actor_index+1,"""eval("all_Actr(t ,o, batch_size)")"""  ))
    ...
    for actor_idx in range(ac_kwargs["num_actor"]):
        exec("globals()['popsan_mean_optimizer%d'] = %s" % (actor_idx + 1, """Adam(eval("popsan_params%d"%(actor_idx + 1)), lr=popsan_lr)"""));
        exec("globals()['pi_std_optimizer%d'] = %s" % (actor_idx + 1, """Adam(eval("ac.popsan%d.log_std_network.parameters()"%(actor_idx + 1)), lr=q_lr)""" ));
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            print("Model: ", model_idx, " Steps: ", t + 1, " Mean Reward: ", test_mean_reward, "Selected Actor ",idx+1)
...
if __name__ == '__main__':
    ...
    for num in range(START_MODEL, START_MODEL + NUM_MODEL):
        ...
        spike_sac(lambda: gym.make(args.env), actor_critic=SpikeActorDeepCritic, ac_kwargs=AC_KWARGS,
                  popsan_lr=1e-4, gamma=0.99,  batch_size=args.batch_size, seed=seed, steps_per_epoch=args.steps_per_epoch, 
                  epochs=args.epochs, norm_clip_limit=3.0, tb_comment=COMMENT, model_idx=num)
