...
class ActorPolicy(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        if not deterministic:
            ...
            actions = th.nn.functional.gumbel_softmax(latent_pi, tau=1, hard=True) * th.linspace(-1, 1, steps=latent_pi.shape[-1]).to(self.device)
            prob = th.nn.functional.softmax(latent_pi, dim=-1)[:, actions.argmax(dim=-1)]
            actions = actions.sum(dim=-1, keepdim=True)
            
        else:
            actions = th.argmax(latent_pi, dim=-1, keepdim=True)
            prob = th.nn.functional.softmax(latent_pi, dim=-1)[:, actions.squeeze(-1)]
        
        # actions = self.value_net(latent_pi)
