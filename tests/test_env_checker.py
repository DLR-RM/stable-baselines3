...
from stable_baselines3.common.env_checker import check_env, check_vecenv
...
def test_check_vecenv_not_vecenv():
    ...
    with pytest.raises(AssertionError, match="must inherit from.*VecEnv"):
...
def test_check_vecenv_missing_attributes():
    ...
    with pytest.raises(AssertionError, match="must have.*attribute"):
        check_vecenv(broken_env)
